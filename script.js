...
function initializeMap() {
    var map = L.map('map').setView([35.5355, -93.4011], 17);
    ...
    fetch('Cleaned_Analysis_For_Leaflet.csv')
        ...
        .then(data => {
            ...
            csvData.forEach(row => {
                ...
                // On marker click, show the FVR chart
...
function showFVRChart(row) {
    let fvrData = [
        parseFloat(row.Week_1_FVR), parseFloat(row.Week_2_FVR), parseFloat(row.Week_3_FVR), parseFloat(row.Week_4_FVR),
        parseFloat(row.Week_5_FVR), parseFloat(row.Week_6_FVR), parseFloat(row.Week_7_FVR), parseFloat(row.Week_8_FVR),
        parseFloat(row.Week_9_FVR), parseFloat(row.Week_10_FVR), parseFloat(row.Week_11_FVR)
    ];

    let ctx = document.getElementById('fvrChart').getContext('2d');
    if (window.myChart) {
        window.myChart.data.datasets[0].data = fvrData;
        window.myChart.data.datasets[0].label = `FVR for Plot: ${row.Plot}`;
...
function createLegend(map) {
    ...
    legend.onAdd = function () {
        ...
        // Loop through the FVR intervals and generate a label with a colored square for each interval
